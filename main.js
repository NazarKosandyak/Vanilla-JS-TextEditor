...
let counter = 1
// Робить текст жирним, міг зробити просто через checkbox ,але пішов іншим шляхом
...
// TEXT ALIGN
// Проделегував події , тобто, замість того, щоб робити  на кожний елемент подію, я на натиснутий елемент зчитую його атрибут 
...
//(Вибір шрифта) В select є свій метод options , отже завдяки йому , я можу получити value вибраного елемента
...
// Делегування події, ПИТАННЯ(якщо я дустаю color,все працює, якщо я хочу дістати backgroundColor , колір не вертає , вертає rgb(0,0,0))
...
// Кнопка закриття, якщо користувач передумав вносити зміни
...
// GALLARY Відкриття головного вікна
...
//Делегування для картинки фону

//Подія відкриття вікна Image 
...
//Подія віркитя вікна Files
openFiles.addEventListener('click', function () {
    getSetFiles.style.display = 'flex'
    getSetColor.style.display = 'none'
    getSetImages.style.display = 'none'
})
// Доступився до кнопки, щоб завантаити файл
let getFiles = document.querySelector('.myFile')
// Получив ім'я моєї картинки , ну підставив його в url
...
// GALLARY Закриття головного вікна
...
btnSub.addEventListener('click', function (e) {
    ...
    if (regName != null && regPass != null) {
        ...
        // Стилізую кнопку (та, що переходить в меню редагування)
        ...
        // кнопка, яка збрегігає редаговане і переходить в головне меню
    ...
    //  Перевіряє, чи значння введені по патерну
...
myFormCreateTable.add.addEventListener('click', function (e) {
    e.preventDefault()
    // Доступу до значень, введиних у таблиці
    ...
    // Створення таблиці за допомогою вложених циклів
...
//    Створю номерований список ( Робота з Ol)
...
// кнопка скидування вмісту форми з (List UL)
